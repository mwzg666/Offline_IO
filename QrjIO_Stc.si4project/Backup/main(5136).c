...
{
    // 当前只有一个开关机状态 P5.0
    BYTE Pin_pc = 0x0B;//0xB3;
    BYTE Pin_almon = 0x03;    
    BYTE st,st1,st2,st3;
    
//    st = ((((BYTE)PC_STAUTUS()) <<   7) & Pin);
//    st |= ((((BYTE)ALARM_CFM()) << 1) & Pin);
//    st |= (((BYTE)ONOFF_LOCK()) & Pin);

    st1 = (((BYTE)PC_STAUTUS()) & Pin_pc);
    st1 <<= 4;
    st2 = (((BYTE)ALARM_CFM()) & Pin_almon);
    st2 <<= 1;
    st = st1 | st2;
    st3 = (((BYTE)ONOFF_LOCK()) & Pin_almon);
    st |= st3;
    return st;
}
